...
import { Building, Review, CATEGORIES } from '@/lib/types';
...
export default function BuildingPage() {
  ...
  CATEGORIES.forEach((c) => {
    const vals = reviews.map((r) => (r as any)[`rating_${c}`]).filter(Boolean);
    avgCats[c] = vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : 0;
  });
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-6">
      ...
      <div className="bg-navy-50 rounded-xl p-4">
        ...
        <CategoryRatings ratings={avgCats} />
      </div>
